...
export const ShareModalMortgage = ({
  ...
  const generateShareableContent = () => {
    const content = `🏠 Mortgage Calculator Results
    
    💰 Monthly Payment: ${formatCurrency(calculation.monthlyPayment)}
    
    📊 Payment Breakdown:
    • Principal & Interest: ${formatCurrencyDetailed(calculation.principal + calculation.interest)}
    • Property Tax: ${formatCurrencyDetailed(calculation.propertyTax)}
    • Home Insurance: ${formatCurrencyDetailed(calculation.insurance)}${
      calculation.pmi > 0
        ? `
    • PMI: ${formatCurrencyDetailed(calculation.pmi)}`
        : ''
    }${
      calculation.hoa > 0
        ? `
    • HOA Fees: ${formatCurrencyDetailed(calculation.hoa)}`
        : ''
    }
    
    📋 Loan Summary:
    • Home Price: ${formatCurrency(parseFloat(homePrice) || 0)}
    • Down Payment: ${formatCurrency(parseFloat(downPayment) || 0)} (${downPaymentPercent}%)
    ...
    Calculated with Mortgage Calculator App`;
    return content;
  };
